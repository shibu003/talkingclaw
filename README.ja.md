...
- ※ 任意: [GitHub CLI](https://cli.github.com) (`gh`) — GitHub から作業先を clone する時だけ使う。
  未認証なら画面の「GitHub と連携する」か `gh auth login` で通す(トークンは gh が保管し、この部屋は持たない)
- ※ 任意: [herdr](https://github.com/shibu003/herdr) — 端末の艦隊を声から操る機能で使う。**無くても部屋は動く**
...
`/project`(作業先)`/log [n]` `/mute` `/quit`。`CLI_MUTE=1` で無音運用。
...
cd ~/any-project
npm run cli
/project add                 # 今いる場所をそのまま作業先にする(パスを打たない)
/project clone owner/repo    # GitHub から取ってきて作業先にする(gh の認証を使う)
/project                     # 一覧(今いる場所には「← 今ここ」が付く)
/project rm <名前>           # 登録だけ外す(フォルダは消えない)
```

フォルダの大きさは関係ない(場所を渡すだけでコピーしない)。同じことは画面からもできる
(設定 → 作業先プロジェクト: GitHub の一覧から選ぶ / フォルダを画面に落とす / 辿って選ぶ)。

**マイクは terminal でも使える** — `npm run setup-stt` で `tools/claw-listen` を一度ビルドすると、
`/v`(1 回話す)と `/v on`(ハンズフリー)が使える(macOS の音声認識)。
ビルドしない場合の音声入力はブラウザ側(Web Speech API)になる。

## 部屋に体を持たせる(VRM)

喋りながら動くキャラを部屋に立てられる。**モデルはこの repo に入っていない** ——
自分の `.vrm` を `~/.talkingclaw/avatars/` に置くと、ヘッダに 🧍 が出る(広い画面なら勝手に開く)。
置かなければ画面は今までどおり。
...
- **[Open Source Avatars](https://www.opensourceavatars.com/)** —— CC0 として公開されている VRM が数百体
  (再配布する時は各モデルのページを確認する)
- **VRoid のサンプルモデル**(`AvatarSample_A/B/C`) —— 使えるし改変もできるが **CC0 ではない**。
  著作権は pixiv にあり[利用条件](https://vroid.pixiv.help/hc/ja/articles/4402394424089)が付く
- **[VRoid Studio](https://vroid.com/studio)** —— 自分で作る

描画は [three.js](https://threejs.org) + [@pixiv/three-vrm](https://github.com/pixiv/three-vrm)(どちらも MIT)。
**ブラウザ側だけの依存**で、部屋のサーバ側の依存は増えていない。
